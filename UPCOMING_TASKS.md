 ...
 **Trigger Mechanism:**
   1 Once user ends the appointment, system prompts the user to rate their experience. [NotDone]
   2 Prompt is not mandatory, user can skip it. [NotDone]
**Firebase Update:**
   1 Update Firestore User Rating Document  
   2 $New Fields: `userRating` (number), `userFeedback` (string)
   3 Update Firestore Expert Document with `$With Fields`
   4 Update Appointment Document `userRating` (number), `expertRating` (number), `expertFeedback` (string), `userFeedback` (string)
 **User Interface (UI):**
 ...
 **Data Persistence:**
      ...
      New Fields: `userRating` (number), `expertRating` (number), `expertFeedback` (string), `userFeedback` (string).
Confirmar cita basarse en el experto ( el experto confirmará)
...
3 *Outcome:* The user is technically cleared to book a new appointment, and the Blocking Logic (Step 3) 
ensures they get a fresh expert match.
