<template>
  <div class="box-border p-2 m-4 w-[95%] bg-emerald-50 rounded-lg border border-sky-700 shadow-xl sm:w-auto">
    <div class="flex flex-row flex-wrap items-center p-4 bg-white rounded-lg">
      ...
      <div class="mr-4">
        <img v-if="expertImage !== ''" :src="expertImage" alt="Expert Profile"
      ...
      <div class="flex flex-col">
        <!-- Expert Name -->
        <h2 class="pb-2 text-xl font-bold text-gray-800 border-b border-sky-600">{{ expertName || 'Unknown Expert' }}
        </h2>
...
const viewSchedule = async () => {
  try {
    ...
    sysStore.setSelectedExpertUid(props.docRef);
